...
namespace GroupGSteganography.Model.Embedding
{
    ...
    public class TextEmbeddor : IEmbeddor
    {
            ...
            for (var i = 0; i < img.Width; i++)
            {
                for (var j = 0; j < img.Height; j++)
                {
                    if (i != 0 || j != 0)
                    {
                        ...
                        this.checkForEndOfMessage(i, img, j, pixel);
                    }
                    else
                    {
        ...
        private void checkForEndOfMessage(int i, Bitmap img, int j, Color pixel)
        {
            if (this.MessageText.Length < i*img.Width + j &&
                i*img.Width + j <= this.MessageText.Length + 3)
            {
                setEndOfMessageCharacters(i, img, j, pixel);
            }
        }

        private static void setEndOfMessageCharacters(int i, Bitmap img, int j, Color pixel)
        {
